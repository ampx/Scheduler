...
public class RequestManager {
    ...
    public boolean deleteUserRequest(Request request){
        /*HashMap<String, Object> options = request.getOptions();
    ...
    //Find original submit request
    //will return cached table if the submit request has complete status and
    //cache result request user has privilege to run original submit request
    ...
    public Table systemRequest(Request request) {
        /*Table response = null;
        if (jobManager.processPermission(request.getRequestUser(), "admin")) {
            HashMap<String, Object> options = request.getOptions();
            if (request.isRequestsRequest()) {
            ...
            } else if (request.isMetricsRequest()) {
